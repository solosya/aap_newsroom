...
export const Donations = function (Stripe, params) {
  ...
  if (this.guest !== "1") {
    this.fetchUser().done(function (r) {
      if (r.success === 1) this.user = r.self;
    });
  }
  this.templates = {
    donations: "donations",
...
Donations.prototype.fetchPrices = function () {
  ...
  for (let i = 0; i < this.products.length; i++) {
    if (
      typeof this.products[i].metadata.active !== "undefined" &&
      this.products[i].metadata.active !== "true"
    ) {
      continue;
    }
...
Donations.prototype.parsePrices = function (r) {
  for (let idx = 0; idx < this.products.length; idx++) {
    ...
    if (
      typeof product.metadata.active === "undefined" ||
    ...
    for (price in product.prices) {
      ...
      if (
        typeof price.metadata.default !== "undefined" &&
        price.metadata.default === "true"
      ) {
      ...
      if (
        this.selectedAmount === newPrice.unit_amount &&
...
Donations.prototype.render = function (layout, data) {
  ...
  if (this.renderTo === "modal") {
    ...
    var wrapper =
      '<div class="{{name}}__content-window" id="dialogContent" style="scrolling == unusable position:fixed element">';
    var endWrapper = "</div>";
    var tmp = Handlebars.compile(
      wrapper + Templates[this.templates[layout]] + endWrapper
    );
    var layout = tmp(data);
    ...
    amountInput.oninput = function (e) {
      ...
      if (priceButtons) {
        for (let i = 0; i < priceButtons.children.length; i++) {
          priceButtons.children[i].classList.remove(
            "donate-form__price-button--active"
          );
        }
      }
  ...
  if (behaviour === "forgot") {
    ...
    self
      ...
      .done(function (r) {
        if (r.success === 1) {
          text.innerHTML =
            "<strong>A password reset link has been sent to your email.</strong> <br />After reset, enter your new password to continue.";
        } else {
          return {
            success: r.success,
            error: r.error.email,
...
Donations.prototype.signin = function (elem) {
  ...
  var text =
    "<strong>It looks like you already have a Newsroom account!</strong> <br />Please enter your password to continue.";
  textElem.innerText = "";
  ...
  if (action === "signin") {
    ...
    Server.create("/api/auth/login", loginData)
      .done(function (r) {
        console.log(r);
        if (r.success === 1) {
...
Donations.prototype.forgot = function () {
  var self = this;
  return Server.create("/api/auth/forgot-password", {
